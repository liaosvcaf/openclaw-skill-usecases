...
**Profile:** Senior Computer Scientist @ LLNL | SVCAF Founding President | WeChat公众号 AI Tech/Policy
...
2. **LDRD Proposal Assistant** — Leverage existing skill for whitepaper generation (15 min)
...
5. **Research Collaboration Coordinator** — LLNL team coordination + literature tracking (1 hour)
...
7. **ROSE Compiler Integration** — AI-assisted refactoring workflows (4-8 hours, experimental)
...
**Use Case / 场景:**
- 🇨🇳 监控LLNL集群上的长时间运行任务（ROSE编译测试、性能基准），任务完成/失败时通过Telegram告警
- 🇺🇸 Monitor long-running jobs on LLNL clusters (ROSE compiler tests, benchmarks), alert via Telegram on completion/failure

**Implementation / 实施步骤:**

```bash
# On LLNL cluster (via SSH from OpenClaw):
# 1. Setup SSH key auth to cluster (ssh ubuntu-rog or LLNL login node)
...
export default {
  ...
  async run({ exec, message }) {
    const result = await exec({
      command: "ssh llnl-login 'squeue -u liao'",
      timeout: 10000
    });
...
**Use Case / 场景:**
- 🇨🇳 为ROSE项目PR自动运行静态分析（clang-tidy, cppcheck），将结果汇总并评论到GitHub PR
- 🇺🇸 Auto-run static analysis (clang-tidy, cppcheck) on ROSE PRs, summarize findings, comment on GitHub
...
# 1. Setup GitHub webhook for chunhualiao/rose
...
#!/bin/bash
PR_BRANCH=$1
git clone --depth 1 --branch $PR_BRANCH https://github.com/chunhualiao/rose.git /tmp/rose-pr
cd /tmp/rose-pr
...
### 2.3 ROSE Compiler Integration: AI-Assisted Refactoring / ROSE编译器AI辅助重构

**Use Case / 场景:**
- 🇨🇳 利用ROSE AST分析能力，结合GPT-4建议重构模式（例如：检测可并行化循环，生成OpenMP指令）
- 🇺🇸 Leverage ROSE AST analysis + GPT-4 to suggest refactoring patterns (e.g., detect parallelizable loops, generate OpenMP directives)
...
- 🔬 **EXPERT:** ROSE API knowledge (Leo already has this!)
...
### 4.1 LDRD Proposal Generation / LDRD提案生成

**Use Case / 场景:**
- 🇨🇳 使用现有`ldrd-proposal-writer`技能，结合LLNL战略重点，快速生成高质量白皮书草稿
- 🇺🇸 Use existing `ldrd-proposal-writer` skill, aligned with LLNL strategic priorities, to rapidly generate high-quality whitepaper drafts
...
You: "Write LDRD proposal: 'AI-Accelerated Compiler Optimization for Exascale Heterogeneous Systems', 
     Category: HPC + AI, 
     Duration: 3 years, 
     My expertise: ROSE compiler, LLVM, source-to-source transformation"
...
OpenClaw:
  ...
  → Generates LaTeX version using LLNL template
```
...
- LDRD proposals are high-stakes, time-intensive
...
# 2. Filter by keywords: "compiler", "optimization", "HPC", "ROSE", "LLVM"
...
### 4.3 Collaboration Coordination for LLNL Teams / LLNL团队协作协调
...
```bash
# Integration with existing LLNL tools (Confluence, JIRA, email)
...
#    - Decisions made
#    - Open questions
# 3. Creates/updates tracking document: memory/llnl-collab/2026-Q1.md
# 4. Sets reminders for action item deadlines
# 5. Generates weekly "What's Blocking Us" summary

# Example:
You: "Meeting notes attached: ROSE-HPC integration discussion"

OpenClaw:
  → Parses notes
  → Extracts:
     • ACTION: Leo - Benchmark ROSE on new Lassen nodes (Due: Feb 12)
...
**Use Case / 场景:**
- 🇨🇳 监控chunhualiao/rose和liaosvcaf repositories，新issue自动分类（bug/feature/question），高优先级通过Telegram告警
- 🇺🇸 Monitor chunhualiao/rose & liaosvcaf repos, auto-triage new issues (bug/feature/question), high-priority alerts via Telegram
...
Link: github.com/chunhualiao/rose/issues/1234
...
**Use Case / 场景:**
- 🇨🇳 监控ROSE CI/CD流水线（GitHub Actions），构建失败时发送Telegram告警，包含日志摘要和可能原因
- 🇺🇸 Monitor ROSE CI/CD pipeline (GitHub Actions), send Telegram alerts on build failures with log summary & likely causes
...
Logs: github.com/chunhualiao/rose/actions/runs/98765
```
...
- ✅ Setup LDRD proposal writer (0 min - already exists!)
- ⚙️ Configure bilingual content pipeline (article-illustrator + translation)
- ⚙️ Setup GitHub issue triager for one repo (chunhualiao/rose)
...
- **Light usage:** $5-15/month
  - LDRD proposals: 2-3/month × $0.50 = $1.50
...
### LLNL-Specific
- LDRD Program: https://ldrd-annual.llnl.gov/
- ROSE Compiler: https://github.com/rose-compiler/rose
- LLNL HPC Docs: https://hpc.llnl.gov/
...
1. **Today (15 min):** Test LDRD proposal writer with a sample topic
...
- Which GitHub repos should be monitored? (chunhualiao/rose, liaosvcaf/*, others?)
- Preferred communication channel? (Telegram, WhatsApp, Discord?)
- LLNL cluster access details? (Login nodes, SLURM vs PBS?)
...
1. ✅ **LDRD Proposal Writer** (15 min setup, 5-10x productivity) — ALREADY INSTALLED!
...
🚀 **Recommendation:** Start with LDRD proposal writer TODAY (zero setup), then add content pipeline this weekend.
